...
- 泛型机制能够对**数据类型**进行约束，避免向下转型机制造成的数据类型转换异常。
   - 泛型类型在**编译**时确定，向下转型机制在**运行**时确定，提高了数据类型的安全性，代码更加简洁、健壮。
```java
public class Test {
    public static void main(String[] args) {
...
- 使用**泛型机制**的类叫做**泛型类**，泛型类的声明方式：`class 类名<T>{}`
   - 泛型类就是具有一个或多个**类型变量**的类，其中**类名<T>**表示泛型类的**类类型**。
```java
...
- 使用**泛型机制**的接口叫做**泛型接口**，泛型接口的声明方式：`interface 接口名<T> {}`
   - 泛型接口就是具有一个或多个**类型变量**的接口，其中**接口名<T>**表示泛型接口的**接口类型**。
```java
...
- 泛型方法被调用时，类型必须确定下来，即编译完成后，确定泛型方法的类型
- 区别：泛型类中的泛型方法和普通方法，一般来讲，泛型类中的泛型方法不使用泛型类的类型
   - 泛型方法可以同时使用泛型类和泛型方法类型
   - 泛型方法与普通方法最本质的区别是，泛型方法有泛型标识符
## 泛型限定符：
...
- <?>：支持任意泛型类型，？位置只能有一个类型
- <? extends A>：支持A以及A的子类(这样写是泛型，而不是多态，注意)，？位置只能有一个类型
- <? super A>：支持A以及A的父类，？位置只能有一个类型
